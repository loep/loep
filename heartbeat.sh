...
for (( ;; )) ; do
	...
	if [ -f /opt/loep_v1.0/conf/general.conf ] ; then
		...
		web_server=`echo -n "$web_server"/heartbeat.html`
		delay=`grep 'update "Network Heartbeat" page after' /opt/loep_v1.0/conf/general.conf | cut -d '=' -f 2 | cut -d ' ' -f 2`
	else
	...
	if [ -d /opt/loep_v1.0/html/ ] ; then
		out='/opt/loep_v1.0/html/heartbeat.html'
	else
	...
	echo '
		...
		<li class="active"><a href="heartbeat.html"><i class="fa fa-fw fa-wrench"></i> Network Heartbeat</a></li>
		...
		<div class="row">
		<div class="col-lg-12">
		<h1 class="page-header">Network Heartbeat</h1>
		<ol class="breadcrumb">
		<li>
		<i class="fa fa-dashboard"></i>  <a href="index.html">Dashboard</a>
		</li>
		<li class="active">
		<i class="fa fa-wrench"></i> Network Heartbeat</li>
		</ol>
	' > $out
	...
	n=`cat /opt/loep_v1.0/conf/heartbeat.conf | wc -l`
	n=`expr $n - 10`
	ip=`cat /opt/loep_v1.0/conf/heartbeat.conf | tail -n $n | cut -d " " -f 1`
	comment=`cat /opt/loep_v1.0/conf/heartbeat.conf | tail -n $n | cut -d " " -f 2-`
	n=`echo "$ip" | wc -l`
	...
	if [ ! -z "$ip" ] ; then
		...
		for (( i=1 ; i <= $n ; i++ )) ; do
			...
			ping -c 2 `echo "$ip" | head -n $i | tail -n 1` &> /dev/null
			if [ "$?" = "0" ] ; then
				echo '
			...
			echo '
				...
				' >> $out
			echo "
				<h3 class="panel-title">`echo "$ip" | head -n $i | tail -n 1`</h3>
				" >> $out
			echo '
				</div>
				' >> $out
			echo "
				<div class="panel-body">`echo "$comment" | head -n $i | tail -n 1`</div>
				" >> $out
			echo '
				</div>
				</div>
		...
		echo '
			...
			<strong>Config file is empty. </strong>Please see /opt/loep_v1.0/conf/heartbeat.conf
			</div>
			' >> $out
	...
	echo '
		<div class="alert alert-danger">
		<strong>Not found : </strong> /opt/loep_v1.0/conf/heartbeat.conf
		</div>
		' >> $out
